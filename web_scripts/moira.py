# Source: https://github.com/sipb/hwops/blob/master/web_scripts/moira.py
...
def scan_acl(acl_name, strip_instance=True):
    try:
        ...
        for line in subprocess.check_output(
            ["/mit/ops/bin/qy", "-n", "-s", "geml", acl_name], stderr=devnull
        ).split("\n"):
            if not line.strip():
                continue
            ...
            if mem_type == "USER":
                ...
                (mem_type == "KERBEROS") and
                (mem_name.endswith("@ATHENA.MIT.EDU"))
            ):
...
def list_exists(list_name):
    try:
        return (
            list_name[0:1].isalpha() and
            (
                subprocess.check_output(
                    [
                        "/mit/ops/bin/qy", "-n", "-s", "-f", "active", "glin",
                        list_name
                    ],
                    stderr=devnull
                ).strip() == "1"
...
def email_to_user(email):
    try:
        kv = subprocess.check_output(
            ["/mit/consult/bin/ldaps", "--", "mail=%s" % email, "uid"],
            stderr=devnull
        ).strip().split("\n")[-1].split(": ", 1)
...
def user_to_email(name):
    try:
        kv = subprocess.check_output(
            ["/mit/consult/bin/ldaps", "--", "uid=%s" % name, "mail"],
            stderr=devnull
        ).strip().split("\n")[-1].split(": ", 1)
        return (kv[1:] and kv[1]) or None
    except subprocess.CalledProcessError:
