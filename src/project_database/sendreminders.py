...
REMIND_DAYS = {
    datetime.timedelta(days=(EXPIRATION_BY_NUM_DAYS - d))
    for d in [30, 21, 14, 7, 3, 2, 1]
}
...
def main():
    ...
    * For projects which are stale, set the project status to "inactive"
    ...
    projects_needing_edits = db.get_stale_projects(
        now, time_horizon=EXPIRATION_HORIZON - max(REMIND_DAYS)
    )
    for project in projects_needing_edits:
        last_edit_age_rounded = round_timedelta(now - project["last_edit_timestamp"])
    ...
    stale_projects = db.get_stale_projects(now, time_horizon=EXPIRATION_HORIZON)
