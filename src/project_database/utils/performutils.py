...
from . import authutils, formutils, strutils, templateutils, valutils
...
def check_for_name_change(project_info, project_id):
    ...
    """
    return project_info["name"].lower() != db.get_project_name(project_id).lower()
...
def check_for_info_change(project_info, project_id):
    ...
    if db_project_info["description"].lower() != project_info["description"].lower():
        return True
    ...
    if len(db_project_info["links"]) != len(project_info["links"]):
        ...
        for db_link, link in zip(db_project_info["links"], project_info["links"]):
    ...
    if len(db_project_info["comm_channels"]) != len(project_info["comm_channels"]):
        return True
    else:
    ...
    if len(db_project_info["roles"]) != len(project_info["roles"]):
        ...
        for db_role, role in zip(db_project_info["roles"], project_info["roles"]):
            ...
            if not nullable_case_insensitive_equals(db_role["prereq"], role["prereq"]):
                return True
...
def format_success_page(project_id, operation, message=None):
    ...
    project_info = authutils.enrich_project_list_with_permissions(user, [project_info])[
        0
    ]
    ...
    result = (
        jenv.get_template("results/performsuccess.html")
        .render(
...
def format_failure_page(status, operation):
    ...
    result = (
        jenv.get_template("results/performfailure.html")
        .render(
            status=status,
            operation=operation,
            user=user,
...
def edit_confirm_main(task, arguments):
    ...
    if is_ok:
        ...
        page = format_success_page(project_id, f"{task} Project", message=message)
    else:
